...
class Yolo(object):
    def __init__(self, weights_file, verbose=True):
        ...
        self.x_offset = np.transpose(
            np.reshape(
                np.array([np.arange(self.S)] * self.S * self.B),
    ...
    def _build_detector(self):
        ...
        boxes = tf.stack(
            [
                (boxes[:, :, :, 0] + tf.constant(self.x_offset, dtype=tf.float32))
                / self.S
                * self.width,
                (boxes[:, :, :, 1] + tf.constant(self.y_offset, dtype=tf.float32))
                / self.S
                * self.height,
        ...
        nms_indices = tf.image.non_max_suppression(
            _boxes, scores, self.max_output_size, self.iou_threshold
        )
    ...
    def _conv_layer(self, x, layer_id, num_filters, filter_size, stride):
        ...
        weight = tf.Variable(
            tf.truncated_normal(
                [filter_size, filter_size, in_channels, num_filters], stddev=0.1
            )
        )
        bias = tf.Variable(
            tf.zeros(
        ...
        conv = tf.nn.conv2d(
            x_pad, weight, strides=[1, stride, stride, 1], padding="VALID"
        )
        output = leak_relu(tf.nn.bias_add(conv, bias))
        if self.verbose:
            print(
                "Layer %d: type=Conv, num_filter=%d, filter_size=%d, stride=%d, output_shape=%s"
    ...
    def _fc_layer(self, x, layer_id, num_out, activation=None):
        ...
        if self.verbose:
            print(
                "Layer %d: type=Fc, num_out=%d, output_shape=%s"
    ...
    def _maxpool_layer(self, x, layer_id, pool_size, stride):
        ...
        if self.verbose:
            print(
                "Layer %d: type=MaxPool, pool_size=%d, stride=%d, output_shape=%s"
    ...
    def detect_from_file(
        ...
        self.show_results(
            image, predict_boxes, imshow, deteted_boxes_file, detected_image_file
        )
    ...
    def show_results(
        ...
        for i in range(len(results)):
            ...
            if self.verbose:
                print(
                    "class: %s, [x, y, w, h]=[%d, %d, %d, %d], confidence=%f"
